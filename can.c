...
unsigned char can_receive(void)
{
	...
	if (RXB0FUL) /* CheckRXB0 */
	{
        ...
        rcv_data[0] = can_payload[D0];
		rcv_data[1] = can_payload[D1];
		rcv_data[2] = can_payload[D2];
		rcv_data[3] = can_payload[D3];
		rcv_data[4] = can_payload[D4];
		rcv_data[5] = can_payload[D5];
		rcv_data[6] = can_payload[D6];
		rcv_data[7] = can_payload[D7];
...
void can_transmit(void)
{
	...
	TXB0D0 = u[0];			/* DataByte 0 */
	TXB0D1 = u[1];			/* DataByte 1 */
	TXB0D2 = u[2];			/* DataByte 2 */
	TXB0D3 = u[3];			/* DataByte 3 */
	TXB0D4 = n[4];			/* DataByte 4 */
	TXB0D5 = n[5];			/* DataByte 5 */
	TXB0D6 = n[6];			/* DataByte 6 */
	TXB0D7 = n[7];			/* DataByte 7 */
